...
const WhyUs = () => {
  return (
    <section id="about" className="py-32 bg-[#0a0a0a] relative border-b border-white">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="bg-dark-bg p-8 lg:p-10 border border-white/5 hover:scale-[1.02] hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(255,255,255,0.07)] hover:border-white/20 transition-all duration-500 flex flex-col h-full group"
            >
              <div>
                {reason.icon}
                <p className="text-gold text-xs tracking-[0.2em] font-bold uppercase mb-2">
                  {reason.title}
                </p>
                <h3 className="text-2xl font-display mb-5 font-medium tracking-wide">
                  {reason.subtitle}
                </h3>
                <p className="text-gray-400 font-light leading-relaxed text-sm lg:text-base mb-8">
                  {reason.description}
                </p>
              </div>
